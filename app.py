...
def analyse():
    ...
    recs = build_recommendations(feats, score, breach_result, profile)
...
@app.route('/api/stats')
def api_stats():
    total    = PasswordLog.query.count()
    breached = PasswordLog.query.filter_by(breach_exposed=True).count()

    dist = db.session.query(
        PasswordLog.strength_label, func.count(PasswordLog.id)
    ).group_by(PasswordLog.strength_label).all()

    patterns_raw   = db.session.query(PasswordLog.pattern_flags).all()
    pattern_counts = {}
    ...
    avg_entropy = db.session.query(func.avg(PasswordLog.entropy)).scalar() or 0
...
def _export_csv(data):
    ...
    writer.writerow(['BambiVault — Anonymised Statistics Export'])
...
def _export_txt(data):
    lines = [
        "BAMBIVAULT — ANONYMISED STATISTICS EXPORT",
        "=" * 45,
        f"Range: {data['range']}",
...
def _export_pdf(data):
    ...
    story = [
        Paragraph("BambiVault — Anonymised Statistics Export", styles['Title']),
...
def _export_docx(data):
    ...
    doc = Document()
    doc.add_heading('BambiVault — Anonymised Statistics Export', level=1)
